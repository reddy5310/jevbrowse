...
| Shortcut/zoom page-message security fix (per-renderer token) | Done, **pending independent security review** | `WebView2LeaseManager` (`ChordToken`/`StripChord`); interaction pass rows 24-25 (negative checks: a page/frame cannot forge a message, and cannot intercept/replay a genuine one, including cross-origin). A postMessage-interception gap was found real and fixed; `docs/security/CHORD_TOKEN_REVIEW.md`. Requested review not yet performed by anyone outside this repository |
| Packaged smoke, engine checks (`--additions-check`) | **Reproducible failure, root cause unresolved** | A `ClickDownload` bench helper fails reproducibly (4/4) on a null script result; confirmed unrelated to the security fix by mutation test, one leading hypothesis (a crashed/replaced renderer) ruled out by a diagnostic log left in place; cause still unknown. 23 of 31 steps in this bench never ran, including every Private-session-specific one, which have no other automated coverage — clean-Windows checklist rows 9 and 16 now ask a tester to confirm Cancel, Continue and session cleanup by hand. See `docs/releases/0.1.0-alpha.7.md` |
