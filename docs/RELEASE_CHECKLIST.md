...
| Packaged smoke, engine checks (`--additions-check`) | **Flaking, unrelated to the security fix (mutation-tested)** | A `ClickDownload` bench helper fails reproducibly on a null script result; the real download feature (row 16c) and the real Private-download prompt (clean-Windows checklist row 9) are unaffected. Root cause not yet found; see `docs/releases/0.1.0-alpha.7.md` |
