...
| **Tested top-level capture blocks automatic hibernation** | **Real engine** | `--media-check`: real WebRTC loopback negotiates and decodes frames (VP8/VP9/H264/AV1 offered); real camera + microphone open through the permission path; the tab reports `MicrophoneActive, CameraActive`, the scheduler is refused **by those flags**, the renderer and both tracks are still live afterwards, and protection clears and sleeping resumes once capture stops | **Top-level document only.** Capture inside an iframe, a real Meet/Zoom session, the screen-share picker and disconnect/reconnect under memory pressure are **not** exercised. Frame aggregation, clone tracking and the `disconnected` recovery window are implemented but only covered by design, not by this check. `SharedArrayBuffer` is off, which some conferencing features want |
