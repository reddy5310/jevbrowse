...
| **Video calls work, and a live call is never hibernated** | **Real engine** | `--media-check`: real WebRTC loopback negotiates and decodes frames (VP8/VP9/H264/AV1 offered); real camera + microphone open through the permission path; the scheduler is then refused (`vetoed by protection: WebRtcActive`) | Loopback, not a real Zoom/Meet call end to end; screen sharing (`getDisplayMedia`) is present but the picker is not driven by the check; `SharedArrayBuffer` is off, which some conferencing features want |
