...
| Reduced transparency | **measured** (ADR 0026): with Windows Transparency effects on and off the window differs in 0.01% of pixels (the live memory digits), title text is 16.7:1 (dark) / 16.5:1 (light) in both, the gate passes in both. Setting restored and read back |
...
| With the sidebar hidden, the palette and help buttons are not on screen | **resolved in ADR 0024**: a toolbar More menu holds both; Ctrl+K and F1 still work |
