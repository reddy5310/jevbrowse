...
public static class WelcomePage
{
    ...
    public static string Html(bool aiConfigured, bool jevConfigured) => $$"""
        ...
        <style>
          :root { --bg:#0b0d12; --panel:#121620; --line:#1e2533; --text:#e6e9ef; --muted:#8b94a7; --teal:#20d6c6; --violet:#8b7bff; --amber:#ffb454; --rose:#ff5d8f; }
          * { box-sizing:border-box } html,body { margin:0; background:radial-gradient(1200px 600px at 20% -10%, #17223a 0%, var(--bg) 55%), var(--bg); color:var(--text); font:15px/1.55 "Segoe UI Variable", "Segoe UI", system-ui, sans-serif; }
          ...
          .card { background:linear-gradient(180deg, #151a26, #0f131c); border:1px solid var(--line); border-radius:18px; padding:22px 22px 18px; position:relative; transform:translateZ(0) rotateX(0); transition:transform .35s cubic-bezier(.2,.8,.2,1), box-shadow .35s; box-shadow:0 18px 40px -24px rgba(0,0,0,.9), inset 0 1px 0 rgba(255,255,255,.04) }
          ...
          .card h2 { margin:0 0 8px; font-size:19px } .card p { margin:0 0 8px; color:#c9d0dc } .card .k { display:inline-block; padding:2px 8px; border:1px solid var(--line); border-radius:8px; font-family:Consolas, monospace; font-size:12.5px; background:#0b0e15; color:var(--teal) }
          .dots span { display:inline-block; width:10px; height:10px; border-radius:50%; margin-right:8px; box-shadow:0 0 12px currentColor } .hot{color:#3ddc84;background:#3ddc84} .warm{color:var(--amber);background:var(--amber)} .cold{color:#4aa3ff;background:#4aa3ff} .virt{color:#6b7385;background:#6b7385}
          ...
          .row { display:flex; gap:12px; flex-wrap:wrap; margin-top:28px } .btn { padding:10px 16px; border-radius:12px; border:1px solid var(--line); background:#131826; color:var(--text); text-decoration:none; font-weight:600 } .btn.primary { background:linear-gradient(90deg, var(--teal), var(--violet)); color:#08101a; border:none }
          .fine { color:var(--muted); font-size:13px; margin-top:30px } .ok{color:var(--teal)} .no{color:var(--rose)}
          table { width:100%; border-collapse:collapse; margin-top:6px } td { padding:6px 0; border-top:1px solid var(--line); color:#c9d0dc } td:first-child { color:var(--text); font-weight:600; width:38% }
