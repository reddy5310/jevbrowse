...
public sealed class CheckpointRepository
{
    ...
    public void Upsert(Checkpoint c)
    {
        ...
        cmd.CommandText = """
            INSERT INTO checkpoints (id, url, title, scroll_x, scroll_y, favicon_url, thumbnail_path, captured_at)
            VALUES ($id, $url, $title, $sx, $sy, $fav, $thumb, $at)
            ON CONFLICT(id) DO UPDATE SET url=$url, title=$title, scroll_x=$sx, scroll_y=$sy, favicon_url=$fav, thumbnail_path=$thumb, captured_at=$at
            """;
    ...
    {
        using var cmd = _db.Connection.CreateCommand();
        cmd.CommandText = "SELECT url, title, scroll_x, scroll_y, favicon_url, thumbnail_path, captured_at FROM checkpoints WHERE id=$id";
